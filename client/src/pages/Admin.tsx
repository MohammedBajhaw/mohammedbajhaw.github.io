...
const emptyItem: Record<Exclude<Section, "profile">, Entry> = {
  ...
  publications: { title: "", venue: "", publicationDate: "", status: "", authors: "", summary: "", url: "", sortOrder: 0 },
  experiences: { role: "", organization: "", location: "", startDate: "", endDate: "", isCurrent: false, summary: "", highlights: [], sortOrder: 0 },
  skills: { label: "", category: "", icon: "Sparkles", sortOrder: 0 },
...
const labels: Record<string, string> = { name: "الاسم", professionalTitle: "العنوان المهني", location: "الموقع", email: "البريد الإلكتروني", linkedinUrl: "رابط LinkedIn", bio: "النبذة", institution: "الجامعة", degree: "الدرجة", field: "التخصص", startYear: "بداية", endYear: "نهاية", description: "الوصف", sortOrder: "ترتيب العرض", title: "العنوان", venue: "المؤتمر / المجلة", publicationDate: "التاريخ", status: "الحالة", authors: "المؤلفون", url: "الرابط", role: "المسمى", organization: "الجهة", startDate: "البداية", endDate: "النهاية", summary: "نبذة", highlights: "النقاط البارزة", label: "المهارة", category: "الفئة", icon: "الأيقونة", slug: "رابط المشروع (slug)", subtitle: "العنوان الفرعي", tags: "الكلمات المفتاحية", tools: "الأدوات", outcomes: "النتائج", featured: "مشروع مختار", projectId: "المشروع", alt: "النص البديل للصورة", photoUrl: "رابط الصورة الشخصية" };
...
const fields: Record<Section, string[]> = {
  ...
  publications: ["title", "venue", "publicationDate", "status", "authors", "summary", "url", "sortOrder"],
...
export default function Admin() {
  ...
  const remove = trpc.portfolio.remove.useMutation({ onSuccess: async () => { toast.success("تم حذف العنصر"); setSelected(null); await utils.portfolio.content.invalidate({ type: active }); await utils.portfolio.public.invalidate(); } });
  ...
  const uploadProfilePhoto = trpc.portfolio.uploadProfilePhoto.useMutation({ onSuccess: async (stored) => { if (selected) { setSelected({ ...selected, photoUrl: stored.url, photoKey: stored.key }); toast.success("تم رفع الصورة، اضغط حفظ لتثبيتها في الملف الشخصي"); } } });
  ...
  const handleFile = (file: File, profile = false) => {
    const reader = new FileReader();
    reader.onload = () => {
      const base64 = String(reader.result);
      if (profile) uploadProfilePhoto.mutate({ filename: file.name, mimeType: file.type, base64 });
      else if (editor?.id) uploadProjectImage.mutate({ projectId: editor.id, filename: file.name, mimeType: file.type, base64, alt: `${editor.title} image`, sortOrder: 0 });
  ...
  return <DashboardLayout>{user?.role !== "admin" ? <div className="permission-note"><h2>صلاحية الإدارة مطلوبة</h2><p>سجّل الدخول بحساب مالك الموقع للوصول إلى إدارة المحتوى.</p></div> : <div>
    ...
    <div className="admin-grid"><aside className="admin-list"><div className="admin-panel-title"><strong>العناصر</strong>{active !== "profile" && <button className="text-button" onClick={() => setSelected(emptyItem[active])}>+ إضافة</button>}</div>{content.isLoading ? <div className="admin-row">جارٍ التحميل…</div> : entries.map((item) => <button className={`admin-row ${editor?.id === item.id ? "active" : ""}`} onClick={() => setSelected(item)} key={item.id}><strong>{displayTitle(active, item)}</strong><span>{item.status || item.category || item.organization || item.location || ""}</span></button>)}</aside>
      ...
      {active === "profile" && <div className="upload-box"><p>ارفع صورتك الشخصية من جهازك. تحفظ الصورة في S3، ثم اضغط «حفظ» لاعتمادها.</p><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => event.target.files?.[0] && handleFile(event.target.files[0], true)} /></div>}
      {active === "projects" && <div className="upload-box"><p>ارفع صور المشروع بعد حفظه. سيتم حفظ الصورة في S3 وربطها بهذا المشروع.</p><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => event.target.files?.[0] && handleFile(event.target.files[0])} /></div>}
